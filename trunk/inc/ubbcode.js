...
function AddText(NewCode) {
        if(document.all){
        	insertAtCaret(document.input.log_Details, NewCode);
        	setfocus();
        } else{
        	document.input.log_Details.value += NewCode;
        	setfocus();
        }
}
...
function blogquote(objID,strAuthor,strTime){
	document.input.log_Details.value += "[quote][b]最初由 [color=blue]"+strAuthor+"[/color] 发表于 "+strTime+"：[/b]\n"+document.getElementById(objID).innerText+"[/quote]";
	setfocus();
}

function setfocus() {
        document.input.log_Details.focus();
}